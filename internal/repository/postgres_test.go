...
func TestGetLatestByPair_OrdersByUpdatedAtNotCreatedAt(t *testing.T) {
	truncate(t)
	repo := NewPostgresQuoteRepository(testDB)
	ctx := context.Background()

	// Create two tasks: u1 first, u2 second.
	u1, _, _ := repo.FindOrCreatePending(ctx, "EUR/MXN", nil)
	u2, _, _ := repo.FindOrCreatePending(ctx, "EUR/MXN", nil)

	// Complete u2 first, then u1 after a delay.
	// u1 has earlier created_at but later updated_at.
	price2 := decimal.NewFromFloat(20.0)
	_ = repo.MarkDone(ctx, u2.ID, price2)

	time.Sleep(10 * time.Millisecond)

	price1 := decimal.NewFromFloat(21.0)
	_ = repo.MarkDone(ctx, u1.ID, price1)
	...
	// Must return u1 (later updated_at), not u2 (later created_at).
	if !got.Price.Equal(price1) {
		t.Fatalf("expected price %s (last updated), got %s", price1, got.Price)
	}
}
